...
import { UPLOAD_LIMITS, getFileSizeInMB } from '../../config/uploadConstants';
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      if (file.size > UPLOAD_LIMITS.MAX_FILE_SIZE_BYTES) {
        toast.error(`File size exceeded! Maximum size is ${UPLOAD_LIMITS.MAX_FILE_SIZE_MB}MB. Your file is ${getFileSizeInMB(file.size)}MB.`);
        setUploadFile(null);
        return;
      }
    ...
    if (documents.length === 0) {
      return (
        <div className="text-center py-20 bg-white/50 backdrop-blur-sm border border-slate-200/60 border-dashed rounded-3xl">
          ...
          <p className="text-slate-500 mb-6">Upload your first document to start generating flashcards and quizzes.</p>
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm font-medium transition-all duration-200 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none"
                ...
                <div className="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 hover:border-emerald-400 hover:bg-emerald-50/30 transition-all duration-200">
                  ...
                  <div className="flex flex-col items-center justify-center py-10 px-6">
                    ...
                    <p className="text-sm font-medium text-slate-700 mb-1">
                      {uploadFile ? (
                        <span className="text-emerald-600 truncate max-w-xs block">
                          {uploadFile.name}
                        </span>
                      ) : (
                        <>
                    ...
                    {!uploadFile && (
                      <p className="text-xs text-slate-500">PDF up to {UPLOAD_LIMITS.MAX_FILE_SIZE_MB}MB</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 h-11 px-4 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {uploading ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl p-8">
            ...
            <button
              ...
              disabled={deleting}
              className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors disabled:opacity-50"
            >
              <X className="w-5 h-5" strokeWidth={2} />
            </button>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 h-11 px-4 bg-linear-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-red-500/25 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {deleting ? (
                  <span className="flex items-center justify-center gap-2">
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
