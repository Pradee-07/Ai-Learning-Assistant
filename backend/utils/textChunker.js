...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (currentWordCount + paragraphWordCount > chunkSize && currentChunk.length > 0) {
        ...
        // --- START CORRECTED OVERLAP LOGIC ---
        const prevChunkText = currentChunk.join(' ');
        const prevWords = prevChunkText.split(/\s+/);
        
        // Use negative Math.min to slice from the end of the array
        const overlapText = prevWords.slice(-Math.min(overlap, prevWords.length)).join(' ');

        currentChunk = [overlapText, paragraph.trim()];
        currentWordCount = overlapText.split(/\s+/).length + paragraphWordCount;
        // --- END CORRECTED OVERLAP LOGIC ---
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  const scoredChunks = chunks.map((chunk, index) => {
    ...
    // Normalize by content length (prevents very long chunks from dominating)
    const normalizedScore = score / Math.sqrt(contentWords);

    // Small bonus for earlier chunks (often more introductory/relevant)
